...
VertexBufferPtr RenderSystem::createVertexBuffer(std::vector<vertex>& list_vertices, UINT size_vertex, UINT size_list, void* shader_byte_code, size_t size_byte_shader)
{
...
IndexBufferPtr RenderSystem::createIndexBuffer(std::vector<unsigned int>& index_list, UINT size_list)
{
	IndexBufferPtr ib = nullptr;
	try
	{
		ib = std::make_shared<IndexBuffer>(index_list, size_list, this);
	}
	catch(...) {}
	return ib;
}
